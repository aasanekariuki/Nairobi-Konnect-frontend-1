...
const schema = z.object({
	...
	mpesaNumber: z
		.string()
		.length(10, 'M-Pesa number must be exactly 10 digits'),
});
...
const RouteCard = ({
	...
	return (
		<div
			className={`w-full p-5 transition-transform transform cursor-pointer lg:w-1/3 hover:scale-105 ${
				isBooked ? 'opacity-50 cursor-not-allowed' : ''
			}`}
		>
			<div className="overflow-hidden rounded-lg shadow-md bg-black">
				<div className="p-6">
					<h2 className="mb-3 text-xl font-semibold text-center text-white">
						{route}
					</h2>
			...
			<div className="flex flex-wrap justify-center gap-5 mt-14">
				{routes.map((route, index) => (
					<RouteCard
						...
						{...route}
						onBook={(route, selectedTime, price) =>
							handleOpenModal(route, selectedTime, price)
						}
